...
class User extends Table {
    ...
    // On se connecte à la BD au moment où crée une instance de User
    ...
    public function add($login, $password, $droits, $prenom, $nom, $email, $tel, $ecole, $annee) {
        $query = "Insert into Membre(login,password,droits,prenom,nom,email,telephone,ecole,annee) 
                      values ('$login',PASSWORD('$password'),'$droits','$prenom','$nom','$email','$tel','$ecole','$annee');";
        mysql_query($query);
    }
    
    public function modify($login, $droits, $prenom, $nom, $email, $tel, $ecole, $annee) {
        $query = "Update Membre
                      Set droits='$droits', prenom='$prenom', nom='$nom', email='$email', telephone='$tel', ecole='$ecole', annee='$annee'
                      where login = '$login';";
        mysql_query($query);
    }
